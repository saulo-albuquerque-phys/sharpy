...
def Phif3hPN(
    ...
    condition = np.logical_or(Lam != 0.0, dLam != 0.0)

    def true_branch(args):
        Lam, dLam, v10, v2, v12, v = args
        tidal = (
            Lam * v10 * (-39.0 / 2.0 - 3115.0 / 64.0 * v2)
            + dLam * (6595.0 / 364.0) * v12
        )
        return tidal

    def false_branch(args):
        Lam, dLam, v10, v2, v12, v = args
        return 0.0 * v  # same shape as tidal

    tidal = lax.cond(
        condition,
        true_branch,
        false_branch,
        (Lam, dLam, v10, v2, v12, v),
    )
