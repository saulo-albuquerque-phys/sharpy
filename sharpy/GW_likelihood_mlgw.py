...
def template_mlgw_bbh(params, time_array, modes):
    ...
    hp, hc            = jax.vmap(gwgen.get_WF, in_axes=(None, 0, None))(theta_mlgw_bbh,jnp.array([time_array]),modes)
...
def project_waveform_mlgw_bbh(params, detector_dictionary, mlgw_dictionary):
    
    f = detector_dictionary.Frequency
    duration=detector_dictionary.T
    #sampling_rate=detector_dictionary.sampling_rate
    ## to subsitute with the precomputed value 
    time_array_mlgw         = mlgw_dictionary["Times_MLGW"]
    segment_length          = mlgw_dictionary["segment_length"]
    n                       = mlgw_dictionary["n_tukey"]
    w                       = mlgw_dictionary["w_tukey"]
    modes_mlgw              = mlgw_dictionary["modes"]
    sampling_frequency      = mlgw_dictionary["sampling_frequency"]
    ...
    h_p                     = h_p*window
    h_c                     = h_c*window
    h_p = jnp.squeeze(h_p, axis=0)
    h_c = jnp.squeeze(h_c, axis=0)
    ...
    freq_array_mlgw         = jnp.fft.rfftfreq(h_p.shape[0],1/sampling_frequency)
    #print("size freq_array_mlgw, hpf, hcf, size f:",[freq_array_mlgw.shape,h_p_f.shape,h_c_f.shape,f.shape])
...
def log_likelihood_det_mlgw_bbh(params, detector_list,mlgw_dictionary):

    log_likelihoods = jax.vmap(single_detector_log_likelihood_mlgw_bbh, in_axes=(None, 0, None))(params, detector_list,mlgw_dictionary)

    # Then use jnp.sum
    return jnp.sum(log_likelihoods)


def single_detector_log_likelihood_mlgw_bbh(params, detector_dictionary, mlgw_dictionary):

    h = project_waveform_mlgw_bbh(params, detector_dictionary, mlgw_dictionary)
    residuals = detector_dictionary.FrequencySeries - h
    return -detector_dictionary.TwoDeltaTOverN * jnp.vdot(residuals / jnp.sqrt(detector_dictionary.sigmasq), residuals / jnp.sqrt(detector_dictionary.sigmasq)).real
